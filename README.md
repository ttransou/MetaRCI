...
- executable structural profiles for document, structured-data, media, and composite;
- representative example records and testbed-aligned profile variants;
- unit-test coverage for profile-contract constraints and profile-specific field behavior.

v0.1 status in this repository:

- The Reference/Context/Interpretive model is implemented and validated.
- The four primary structural profiles are treated as peers in the current model.
- Structured-Data-Relational remains explicitly deferred to a future release stream.
...
MetaRCI draws on established work in metadata design, application profiles, provenance, knowledge organization, semantic enrichment, metadata-aware retrieval, and epistemic governance. These sources do not describe the MetaRCI model itself; they provide the standards, concepts, and empirical findings against which the model is positioned.
...
## Epistemic Framing and AI-Mediated Judgment
...
* Quattrociocchi, W., Capraro, V., & Perc, M. (2025). *Epistemological Fault Lines Between Human and Artificial Intelligence*. arXiv preprint arXiv:2512.19466.
  ...
  Examines structural differences between human judgment and LLM generation across grounding, parsing, experience, motivation, causality, metacognition, and value. The paper is relevant to MetaRCI's epistemic framing rather than to its metadata schema directly: its distinction between linguistic plausibility and epistemic evaluation parallels MetaRCI's effort to preserve distinctions among source-grounded Reference metadata, externally curated Context metadata, and Interpretive metadata. Its emphasis on evidential status, grounding, uncertainty, and epistemic transparency also supports the broader principle that AI-mediated workflows should make the basis and status of information visible rather than collapsing all assertions into an undifferentiated output.
...
The following workstreams are intentionally deferred and are not part of v0.1 completion claims:

- Structured-Data-Relational profile expansion and relational-source testbed closure.
- Additional profile-level rights/licensing structure beyond current base and media support.
- Additional Composite semantics beyond Reference-tier components.

Deferred items remain valuable future work, but are outside the current release boundary.
